...
foreach ($src in $(Get-ChildItem -Path src\ -Directory -Force -ErrorAction SilentlyContinue -Exclude ".vs" | Select-Object FullName)) {
    ...
    if ($suffix) {
        & dotnet pack -c Release --include-source -o ..\..\artifacts --version-suffix=$suffix /p:Version=1.2.3 --no-build
    } else {
        & dotnet pack -c Release --include-source -o ..\..\artifacts /p:Version=1.2.3 --no-build
    }
